def weighted_of_masked_data(data_in,data_mask,data_cond):
...
def analyze_PICES_Region(region,var,initial_date,final_date):
    ...
    if (var=='wind') or (var=='current'):
        ...
        plt.savefig('./User_Data_And_Figures/PICESregion'+str(lmei)+'_'+svar+'_timeseries_'+initial_date+'_'+final_date+'.png')
        plt.show()
        ...
        if (np.sign(dtclim[nvarv].min())!=np.sign(dtclim[nvarv].max())):
            plt.axhline(color='k',zorder=0)
        plt.tight_layout()
        plt.savefig('./User_Data_And_Figures/PICESregion'+str(lmei)+'_'+svar+'_climatology_'+initial_date+'_'+final_date+'.png')
        plt.show()
        ...
        plt.xlabel(svarv+' ('+units+')')
        plt.tight_layout()
        plt.savefig('./User_Data_And_Figures/PICESregion'+str(lmei)+'_'+svar+'_densityplots_'+initial_date+'_'+final_date+'.png')
        plt.show()
